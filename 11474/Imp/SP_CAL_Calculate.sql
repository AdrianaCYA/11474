...
BEGIN
    ...
    /* Inicializacion de las variables globales */
        ...
        BEGIN
            ...
            INSERT INTO CAL_LogCalculate (dateExecute, model, descriptionLog)
        ...
        SELECT	@minPzSKU = paramValue FROM CAL_Params WHERE paramCode = 'MinPzSku' and season = @season AND division = @division
        -- Numero de Tiendas
        SELECT	@numTiendas = paramValue FROM CAL_Params WHERE paramCode = 'NumTiendas' and season = @season AND division = @division
        -- Porcentaje de compra por SKU
        SELECT	@porCompraSKU = paramValue FROM CAL_Params WHERE paramCode = 'PorCompraSku' and season = @season AND division = @division
        SET @porCompraSKU = @porCompraSKU / 100
        -- Porcentaje de Pack B contra Pack A
        SELECT	@porPackBvsPackA = paramValue FROM CAL_Params WHERE paramCode = 'PorPackBvsPackA' and season = @season AND division = @division
        SET @porPackBvsPackA = @porPackBvsPackA / 100
        -- Piezas por pack simple
        SELECT	@pzPackSimple = paramValue FROM CAL_Params WHERE paramCode = 'PzPackSimple' and season = @season AND division = @division
        -- Piezas por Pack A
		...
		-- Piezas por split
		SELECT @PzSplit = paramValue FROM CAL_Params WHERE paramCode = 'PzSplit' AND season = @season AND division = @division
   ...
   /* InicializaciOn de las variables globales */
    ...
    /* INICIO DE SPLITS */
        ...
        CREATE TABLE #TempPorcentaje  (ID INT IDENTITY (1,1), paramCode NVARCHAR(20), paramValue DECIMAL(15,6))
            ...
            BEGIN
                INSERT INTO #TempPorcentaje  SELECT paramCode, CAST(paramValue as DECIMAL) / 100 as paramValue FROM CAL_Params WHERE paramCode LIKE 'PorPedNac%' and season = @season and paramValue > 0  AND division = @division
            END
            ELSE --IF @TIPO = 'IMPORTADO'
            BEGIN
                INSERT INTO #TempPorcentaje  SELECT paramCode, CAST(paramValue as DECIMAL) / 100 as paramValue FROM CAL_Params WHERE paramCode LIKE 'PorPedImp%' and season = @season and paramValue > 0 AND division = @division
            END
